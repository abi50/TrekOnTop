...
public class CityService : ICityService
{
    ...
    public void Delete(int id)
    {
        ...
        if (city == null) throw new Exception($"City with ID {id} not found.");
    ...
    public CityDto Update(int id, CityDto item)
    {
        ...
        if (existingCity == null) throw new Exception($"City with ID {id} not found.");
        return _mapper.Map<CityDto>(_repository.UpdateItem(id, _mapper.Map(item, existingCity)));
    }

    public CityDto GetByName(string name)
    {
        var city = _repository.GetAll().FirstOrDefault(c => c.Name == name );
        return _mapper.Map<CityDto>(city);
    }
}
